...
±±³Fun‡…o    ³ MYCOM02  ³ Autor ³ Alexandre Inacio Lemes³ Data ³06/09/2006³±±
...
/*/
User Function MYCOM02( cAlias, nReg, nOpcx )
	...
	While !oReport:Cancel() .And. !SC7->(Eof()) .And. SC7->C7_FILIAL == xFilial("SC7") .And. SC7->C7_NUM >= mv_par01 .And. SC7->C7_NUM <= mv_par02
		...
		For nVias := 1 to mv_par09
			...
			If !Empty(SC7->C7_APROV)
				...
				cEmailC := UsrRetMail(SC7->C7_USER) 
				If SC7->C7_CONAPRO != "B"
					lLiber := .T.
				EndIf
				dbSelectArea("SCR")
			...
			If !lNewAlc
				...
				EndIf
    			oReport:PrintText(STR0137,,850 ) //"NOTA: So aceitaremos a mercadoria se na sua Nota Fiscal constar o numero do nosso Pedido de Compras."  
			Else
				...
				oReport:SkipLine() 
				oReport:PrintText("Email Comprador Responsavel : "+Substr(cEmailC,1,60),,050 ) 	//"Email Comprador Responsavel :" //"BLQ:Bloqueado"
				oReport:SkipLine()
				...
				If SC7->C7_TIPO == 1 .OR. SC7->C7_TIPO == 3
					...
					//SSILVA
					oReport:PrintText(STR0130,,050 ) //"."   
					oReport:PrintText(STR0131,,050 ) //"."   
					oReport:PrintText(STR0132,,050 ) //"." 

					oReport:SkipLine()				
				Else
					oReport:PrintText(STR0083,,050 ) //"NOTA: So aceitaremos a mercadoria se na sua Nota Fiscal constar o numero da Autorizacao de Entrega."
				EndIf
    			oReport:PrintText(STR0137,,830 ) //"NOTA: So aceitaremos a mercadoria se na sua Nota Fiscal constar o numero do nosso Pedido de Compras."  
			EndIf
...
Static Function CabecPCxAE(oReport,oSection1,nVias,nPagina)
	...
	oReport:PrintText(STR0110+Left(SA2->A2_MUN, 30)+" "+STR0111+SA2->A2_EST+" "+STR0112+SA2->A2_CEP+" "+STR0124+":"+Transform(SA2->A2_CGC,Iif(SA2->A2_TIPO == 'F',Substr(PICPES(SA2->A2_TIPO),1,17),Substr(PICPES(SA2->A2_TIPO),1,21))),nLinPC,1025)
	oReport:SkipLine()
...
Static Function FinalPed(nDescProd)
	...
	Else
		...
		@ li,027 PSAY Substr(cComprador,1,60)+"  Email: "+cEmailc
		@ li,088 PSAY "|"
