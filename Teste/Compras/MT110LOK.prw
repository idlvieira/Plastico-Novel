...
ฑฑบDesc.     ณClasse responsavel pela validacao do preenchimento obrigato-บฑฑ
ฑฑบ          ณrio do campo centro de custos quando tratar-se de despesas. บฑฑ
...
local nPosCapex:= gdFieldPos("C1_MYCAPEX")
...
For nx:=1 To Len(aCols)
	...
	IF (DbSeek(xFilial("SAI")+cUser+cGProd)) && Procuro por Produto
		If  (cGaAprov <> SAI->AI_M_APROV)
			IF 	Empty(cGaAprov)
				cGaAprov := SAI->AI_M_APROV
			Else
