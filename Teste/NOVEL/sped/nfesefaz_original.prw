...
If cTipo == "1"
	...
	//						If MsSeek(xFilial("SD2")+(cAliasSD2)->D2_NFORI+(cAliasSD2)->D2_SERIORI+(cAliasSD2)->D2_CLIENTE+(cAliasSD2)->D2_LOJA+(cAliasSD2)->D2_COD+(cAliasSD2)->D2_ITEMORI)
					...
					//-----------------------------------------------------------------------------------------
					If (SF4->(FieldPos("F4_CONSUMO")) > 0 .And. SF4->F4_CONSUMO == "N") .And. (cOrigem $"1-2-3-4-5-6" .And. cCSTrib $ "00-10-20-70-90") .And. lFCI
		...
		Else				
			...
			While !Eof() .And. xFilial("SD1") == (cAliasSD1)->D1_FILIAL .And.;
				...
				aadd(aProd,	{Len(aProd)+1,;  
					...
					0,;
					"",;
					""})  // Da posiÁ„o 28 a 33 tratamento realizado apenas para documento de saÌda por este motivo campos est„o zerados e vazios.
