...
#INCLUDE "COLORS.CH"
#INCLUDE "TBICONN.CH"  
// ALTERADO PARA NOVEL EM 21/01/2014
...
Private nTotalCrg	:= 0
Private cTpCliente	:= ""
// VARIAVEIS PRIVATE CUSTOMIZADAS - NOVEL
Private lImp:= .F.
_novaux := .T.
...
If cTipo == "1"
	...
	//						If MsSeek(xFilial("SD2")+(cAliasSD2)->D2_NFORI+(cAliasSD2)->D2_SERIORI+(cAliasSD2)->D2_CLIENTE+(cAliasSD2)->D2_LOJA+(cAliasSD2)->D2_COD+(cAliasSD2)->D2_ITEMORI)
					...
					// NOVEL INI - INCLUIDO MENSAGENS DO PEDIDO PARA GERAR NO XML
						IF _novaux
					   	   _novaux := .F.
					...
					If (SF4->(FieldPos("F4_CONSUMO")) > 0 .And. SF4->F4_CONSUMO == "N") .And. (cOrigem $"1-2-3-4-5-6-8" .And. cCSTrib $ "00-10-20-40-70-90")
						...
						Do Case
							...
							Case AllTrim(CD2->CD2_IMP) == "ISS" 
								...
								If Empty(aISS)
									aISS := {0,0,0,0,0}
								EndIf
								//	aISS[01] += (cAliasSD2)->D2_TOTAL+(cAliasSD2)->D2_DESCON NOVEL  17/10/2013
		...
		Else				
			...
			While !Eof() .And. xFilial("SD1") == (cAliasSD1)->D1_FILIAL .And.;
				...
				aadd(aProd,	{Len(aProd)+1,;  
					...
					(cAliasSD1)->D1_VALFRE,;
					(cAliasSD1)->D1_SEGURO,;
					0,; // alteracao  // ((cAliasSD1)->D1_VALDESC + nDescRed),; NOVEL 18/10/2013
				...
				ElseIf !lEasy
					...
					If MsSeek(xFilial("CD5")+(cAliasSD1)->D1_DOC+(cAliasSD1)->D1_SERIE+(cAliasSD1)->D1_FORNECE+(cAliasSD1)->D1_LOJA+(cAliasSD1)->D1_ITEM)
						...
						aAdi := aDI
						lImp:= .T.   //NOVEL - em 30/04/2012 - Wesley - para gravar as informacoes da DI
...
If !Empty(aNota)
        // Novel - Inicio - Adiciona mensagem customizada (Importacao) na variavel Local cMensCli
           cMenscli+=MensOBS(cTipo)
        // Novel - Fim - Adiciona mensagem customizada (Importacao) na variavel Local cMensCli  
	cString := ""
	...
	For nX := 1 To Len(aProd)
		...
		cString += 	NfeItem(aProd[nX],aICMS[nX],aICMSST[nX],aIPI[nX],aPIS[nX],aPISST[nX],aCOFINS[nX],aCOFINSST[nX],aISSQN[nX],aCST[nX],;
					...
					@cMensFis,aCsosn[Nx],aPedCom[nX],aNota[5],aICMSZFM[nX],aDest,cIpiCst,aFCI[nX])
	Next nX
	...
	cString += NfeInfAd(cMensCli,cMensFis,aPedido,aExp,cAnfavea,aMotivoCont,aNota,aNfVinc,aProd,aDI,aNfVincRur,aRetido,cNfRefcup,cSerRefcupc,cTipo,nIPIConsig,nSTConsig)
	cString += "</infNFe>"
EndIf
...
Static Function NfeItem(aProd,aICMS,aICMSST,aIPI,aPIS,aPISST,aCOFINS,aCOFINSST,aISSQN,aCST,aMed,aArma,aveicProd,aDI,aAdi,aExp,aPisAlqZ,aCofAlqZ,aAnfI,cTipo,cVerAmb,aComb,cMensFis,cCsosn,aPedCom,cF2Tipo,aICMSZFM,aDest,cIpiCst,aFCI)
...
Static Function NfeInfAd(cMsgCli,cMsgFis,aPedido,aExp,cAnfavea,aMotivoCont,aNfSa,aNfVinc,aProd,aDI,aNfVincRur,aRet,cNfRefcup,cSerRefcup,cTipo,nIPIConsig,nSTConsig)
...
IF cTipo == "0" .AND. !EMPTY(SF1->F1_NVIMPOB)//SA1->A1_EST == "EX"
	...
	If lImp
		...
		IF Val(aAux[9]) > 0
			cMens += "Imposto de ImportaÁ„o: R$" + ConvType(VAL(aAux[9]),14,2) + "-"
		EndIf
