...
//Local cPercTrib	:= ""   //CUSTOMIZADO ORIGINAL JAG
//Local cTpCliente	:= ""   //CUSTOMIZADO ORIGINAL JAG
Local cMsgDI		:= ""
Local nX         	:= 0
Local nY		 	:= 0
Local nCon       	:= 1  
...
//Local nTotTrib	:= 0 //CUSTOMIZADO ORIGINAL JAG
Local nValIcmDev	:= 0
Local nValIcmDif	:= 0
Local nIPIConsig	:= 0
Local nSTConsig		:= 0
...
Private cTPNota	:= ""	
Private lDifal	:= .F.
// CUSTOMIZADO ORIGINAL LAU E IBI - VARIAVEIS PRIVATE CUSTOMIZADAS - NOVEL 
Private lImp:= .F.
_novaux := .T.        

// CUTOMIZADO ORIGINAL IBI VARIAVEIS PRIVATE CUSTOMIZADAS - NOVEL
//Private lImp:= .F.
Private qTIPO := ""
Private nVez	:= 0 
If FunName() == "SPEDNFSE"
...
// CUSTOMIZADO ORIGINAL IBI - NOVEL INI - VARIAVEIS ESPECIFICAS DO CLIENTE
//_novaux := .T.
// NOVEL FIM	
...
If cTipo == "1"
	...
	If MsSeek(xFilial("SF2")+cNota+cSerie+cClieFor+cLoja)
		...
		ElseIf FunName() == "SPEDNFSE"
			...
			//CUSTOMIZADO///////////////////ORIGINAL JAG
			//aadd(aDest,SA1->A1_EMAIL)          
				aadd(aDest,Trim(getmv("MV_RELACNT"))+";"+SA1->A1_EMAIL)//ORIGINAL JAG
			//////////////////////////////////ORIGINAL JAG
		...
		Else
			...
			If !SF2->F2_TIPO $ "DB" 
				...
				If SF2->(FieldPos("F2_CLIRET"))<>0 .And. !Empty(SF2->F2_CLIRET+SF2->F2_LOJARET) .And. SF2->F2_CLIRET+SF2->F2_LOJARET<>SF2->F2_CLIENTE+SF2->F2_LOJA
				    dbSelectArea("SA1")
					dbSetOrder(1)
					MsSeek(xFilial("SA1")+SF2->F2_CLIRET+SF2->F2_LOJARET)
				...
				If !Empty(AllTrim(SF2->F2_CLIENT)) .And. !Empty(AllTrim(SF2->F2_LOJENT))
					...
					If Len(aEntrega) > 0
						//MsSeek(xFilial("SA1")+SF2->F2_CLIENTE+SF2->F2_LOJA)						
						//Se a UF da entrega for diferente da UF do emitente (operação interestadual), tenho que buscar os dados do cliente de entrega para nao ocorrer 
						//rejeicao 523 - CFOP não é de Operação Estadual e UF emitente igual à UF destinatário
						If aEntrega[08] <> IIF(!lEndFis,ConvType(SM0->M0_ESTCOB),ConvType(SM0->M0_ESTENT))//aEntrega[08] <> Upper(SA1->A1_EST)
							MsSeek(xFilial("SA1")+SF2->F2_CLIENT+SF2->F2_LOJENT)		
						EndIf
						...
						If aEntrega[08] == IIF(!lEndFis,ConvType(SM0->M0_ESTCOB),ConvType(SM0->M0_ESTENT))
							MsSeek(xFilial("SA1")+SF2->F2_CLIENTE+SF2->F2_LOJA)
						EndIf
					Endif
				Else
			    ...
			    /*
				//CUSTOMIZADO ORIGINAL JAG
 				If SF2->(FieldPos("F2_CLIENT"))<>0 .And. !Empty(SF2->F2_CLIENT+SF2->F2_LOJENT) .And. SF2->F2_CLIENT+SF2->F2_LOJENT<>SF2->F2_CLIENTE+SF2->F2_LOJA //ORIGINAL
				    dbSelectArea("SA1")   //ORIGINAL
					dbSetOrder(1)         //ORIGINAL

					MsSeek(xFilial("SA1")+SF2->F2_CLIENT+SF2->F2_LOJENT)    //ORIGINAL
					
					aadd(aEntrega,SA1->A1_CGC)                              //ORIGINAL
					aadd(aEntrega,MyGetEnd(SA1->A1_END,"SA1")[1])           //ORIGINAL
					aadd(aEntrega,ConvType(IIF(MyGetEnd(SA1->A1_END,"SA1")[2]<>0,MyGetEnd(SA1->A1_END,"SA1")[2],"SN"))) //ORIGINAL
					aadd(aEntrega,MyGetEnd(SA1->A1_END,"SA1")[4])           //ORIGINAL
					aadd(aEntrega,SA1->A1_BAIRRO)                           //ORIGINAL
					aadd(aEntrega,SA1->A1_COD_MUN)                          //ORIGINAL
					aadd(aEntrega,SA1->A1_MUN)                              //ORIGINAL
					aadd(aEntrega,Upper(SA1->A1_EST))                       //ORIGINAL
				EndIf                                                       //ORIGINAL
				//CUSTOMIZADO ORIGINAL JAG						
				*/						
			Else
				...
				//CUSTOMIZADO//ORIIGINAL JAG
				//aadd(aDest,SA2->A2_EMAIL)     
					aadd(aDest,Trim(getmv("MV_RELACNT"))+";"+SA2->A2_EMAIL)  //ORIGINAL JAG
				////////////////////////////////                             //ORIGINAL JAG
			...
			While !Eof() .And. xFilial("SD2") == (cAliasSD2)->D2_FILIAL .And.;
				...
				If lContinua
					...
					//CUSTOMIZADO//////////////////////////////////////////////////////////////
						...
						dbSelectArea("SD1")
						_cAliasSD1	:= Alias()     // Mantem o Alias corrente
						_nIndexSD1 	:= IndexOrd()  // Mantem o Indice corrente
						_nRegSD1   	:= RecNo()     // Mantem o Registro corrente
						dbSelectArea("SF1")
						_cAliasSF1	:= Alias()     // Mantem o Alias corrente
						_nIndexSF1 	:= IndexOrd()  // Mantem o Indice corrente
						_nRegSF1   	:= RecNo()     // Mantem o Registro corrente
						dbSelectArea("SA1")
						_cAliasSA1	:= Alias()     // Mantem o Alias corrente
						_nIndexSA1 	:= IndexOrd()  // Mantem o Indice corrente
						_nRegSA1   	:= RecNo()     // Mantem o Registro corrente
						dbSelectArea("SA2")
						_cAliasSA2	:= Alias()     // Mantem o Alias corrente
						_nIndexSA2 	:= IndexOrd()  // Mantem o Indice corrente
						_nRegSA2   	:= RecNo()     // Mantem o Registro corrente
						dbSelectArea("SF2")
						_cAliasSF2	:= Alias()     // Mantem o Alias corrente
						_nIndexSF2 	:= IndexOrd()  // Mantem o Indice corrente
						_nRegSF2   	:= RecNo()     // Mantem o Registro corrente
						dbSelectArea("SD2")
						_cAliasSD2	:= Alias()     // Mantem o Alias corrente
						_nIndexSD2 	:= IndexOrd()  // Mantem o Indice corrente
						_nRegSD2  	:= RecNo()     // Mantem o Registro corrente
             				///////////////////////////////////////////////////////////////////////////
					//ORIGINAL JAG 
					*/ 
	...
	//						If MsSeek(xFilial("SD2")+(cAliasSD2)->D2_NFORI+(cAliasSD2)->D2_SERIORI+(cAliasSD2)->D2_CLIENTE+(cAliasSD2)->D2_LOJA+(cAliasSD2)->D2_COD+(cAliasSD2)->D2_ITEMORI)
					...
					//ORIGINAL JAG
					//CUSTOMIZADO///////////////////////////////////////////////////////////
						DBSelectArea(_cAliasSD2) 		// Volta o Alias corrente
						DBSetOrder(_nIndexSD2)    		// Volta o Indice corrente
						DBGoTo(_nRegSD2)         		// Volta o Registro corrente
						DBSelectArea(_cAliasSF2) 		// Volta o Alias corrente
						DBSetOrder(_nIndexSF2)    		// Volta o Indice corrente
						DBGoTo(_nRegSF2)         		// Volta o Registro corrente
						DBSelectArea(_cAliasSA1) 		// Volta o Alias corrente
						DBSetOrder(_nIndexSA1)    		// Volta o Indice corrente
						DBGoTo(_nRegSA1)         		// Volta o Registro corrente
						DBSelectArea(_cAliasSA2) 		// Volta o Alias corrente
						DBSetOrder(_nIndexSA2)    		// Volta o Indice corrente
						DBGoTo(_nRegSA2)         		// Volta o Registro corrente
						DBSelectArea(_cAliasSF1) 		// Volta o Alias corrente
						DBSetOrder(_nIndexSF1)    		// Volta o Indice corrente
						DBGoTo(_nRegSF1)         		// Volta o Registro corrente
						DBSelectArea(_cAliasSD1) 		// Volta o Alias corrente
						DBSetOrder(_nIndexSD1)    		// Volta o Indice corrente
						DBGoTo(_nRegSD1)         		// Volta o Registro corrente
					...
					*/  
					...
					//CUSTOMIZADO
					// ORIGINAL LAU - IBI - NOVEL INI - INCLUIDO MENSAGENS DO PEDIDO PARA GERAR NO XML
						IF _novaux
					   	   _novaux := .F.
					  	   cMensCli += "" + ALLTRIM(SC5->C5_DESCR6) + "  " + ALLTRIM(SC5->C5_DESCR7) + " "  ;
					 	   + ALLTRIM(SC5->C5_DESCR8)  + " " + ALLTRIM(SC5->C5_DESCR9)  + " " + ALLTRIM(SC5->C5_DESCR10) + " ";
					 	   + ALLTRIM(SC5->C5_DESCR11) + " " + ALLTRIM(SC5->C5_DESCR12) + " " + ALLTRIM(SC5->C5_DESCR13) + " ";
					 	   + ALLTRIM(SC5->C5_DESCR14) + " " + ALLTRIM(SC5->C5_DESCR15)
					 	   cMensFis += " "	+ ALLTRIM(SC5->C5_DESCR1) + " " + ALLTRIM(SC5->C5_DESCR2) + " " ;
					 	   + ALLTRIM(SC5->C5_DESCR3) + " " + ALLTRIM(SC5->C5_DESCR4) + " " + ALLTRIM(SC5->C5_DESCR5)
					 	ENDIF
			        // ORIGINAL LAU - NOVEL FIM   
				...
				//CUSTOMIZADO ////////////////////////////////////////
			    	...
			    	//RestArea(_aAreaAtual)
						...
						Else
							...
							_cCodCli := SF1->F1_FORNECE
							_cLojCli := SF1->F1_LOJA
						Endif

						DBSelectArea("SZZ")
						DbSetOrder(1)
						If DbSeek(xFilial("SZZ") + _cTipoNf + _cNota + _cSerie + _cCodCli + _cLojCli)
							While (SZZ->ZZ_TIPODOC == _cTipoNf .and. SZZ->ZZ_DOC == _cNota .and. SZZ->ZZ_SERIE == _cSerie .and. SZZ->ZZ_CLIFOR == _cCodCli .and. SZZ->ZZ_LOJA == _cLojCli )
								If !(AllTrim(SZZ->ZZ_TXTMENS) $  cMensCli)
			...
			/*CUSTOMIZADO  -  LAU
		            ...
		            Else 
						...
						//	nDesconto := (cAliasSD2)->D2_DESCON // ALTERACAO NOVEL 17/10/2013  
			...
			*/  				 				   			 
					...
					aadd(aProd,	{Len(aProd)+1,;
						...
						(cAliasSD2)->D2_VALFRE,;
						(cAliasSD2)->D2_SEGURO,;
						(nDesconto+nDescIcm+nDescRed),; //*IIF(SA1->A1_ME<>"S",0,(nDesconto+nDescIcm+nDescRed)),;   //0,; // *///SSILVA 20150819 ORIGINAL LAU // VOLTA PADRAO 20160401
	...
	If MsSeek(xFilial("SF1")+cNota+cSerie+cClieFor+cLoja)
		...
		Else				
			...
			If SF1->F1_TIPO $ "DB" 
				...
				//Customizado
				//aadd(aDest,SA1->A1_EMAIL)
					aadd(aDest,Trim(getmv("MV_RELACNT"))+";"+SA1->A1_EMAIL)
				////////////////////////////////
               	//ORIGINAL JAG
				aAdd(aDest, SA1->A1_CONTRIB) // Posição 17
				aadd(aDest,Iif(SA1->(FieldPos("A1_IENCONT")) > 0 ,SA1->A1_IENCONT,""))
			...
			Else
				...
				//Customizado ORIGINAL JAG
				//aadd(aDest,SA2->A2_EMAIL)
					aadd(aDest,Trim(getmv("MV_RELACNT"))+";"+SA2->A2_EMAIL)
				////////////////////////////////
	           	//ORIGINAL JAG
				If SA2->(FieldPos("A2_CONTRIB"))>0
					aadd(aDest,SA2->A2_CONTRIB)
		       ...
		       EndIf   		
				...
				ElseIf !lEasy
					...
					If MsSeek(xFilial("CD5")+(cAliasSD1)->D1_DOC+(cAliasSD1)->D1_SERIE+(cAliasSD1)->D1_FORNECE+(cAliasSD1)->D1_LOJA+(cAliasSD1)->D1_ITEM)
						...
						aAdi := aDI
						//CUSTOMIZADO LAU
						lImp:= .T.   //ORIGINAL LAU - NOVEL - em 30/04/2012 - Wesley - para gravar as informacoes da DI   
...
//					nMargem := IiF(CD2->CD2_PREDBC>0,IiF(CD2->CD2_PREDBC > 100,0,100-CD2->CD2_PREDBC),IiF(Len(aAdI[1])>0 .And. ConvType(aAdI[1][04][01]) == "I19",IiF((aAdi[1][14][03]) > 100,0,aAdi[1][14][03]),CD2->CD2_PREDBC))
			...
			//RestArea(_aAreaAtual)
				...
				SZZ->(DbSelectArea("SZZ"))
				SZZ->(DbSetOrder(1))
				If SZZ->(DbSeek(xFilial("SZZ") + "E" + SF1->F1_DOC + SF1->F1_SERIE + SF1->F1_FORNECE + SF1->F1_LOJA))
					Do While SZZ->(!Eof()) .AND. SZZ->ZZ_FILIAL == xFilial("SZZ");
							.AND. SZZ->ZZ_TIPODOC == "E";
							.AND. SZZ->ZZ_DOC == SF1->F1_DOC;
							.AND. SZZ->ZZ_SERIE == SF1->F1_SERIE;
							.AND. SZZ->ZZ_CLIFOR == SF1->F1_FORNECE;
							.AND. SZZ->ZZ_LOJA = SF1->F1_LOJA
					
						cMensCli += IIf(!Empty(cMensCli),"; ","") + AllTrim(SZZ->ZZ_TXTMENS)
...
If !Empty(aNota)
/*CUSTOMIZADA
// ORIGINAL LAU - Novel - Inicio - Adiciona mensagem customizada (Importacao) na variavel Local cMensCli
           cMenscli+=MensOBS(cTipo)
// ORIGINAL LAU - Novel - Fim - Adiciona mensagem customizada (Importacao) na variavel Local cMensCli  
*/
	cString := ""
...
If !Empty(aIEST) 
	...
	If AllTrim(ConvType(VldIE(SM0->M0_INSC))) <> Alltrim(aIEST[01]) .And. Alltrim (aIEST[01]) <> Alltrim(aIEST[02])
		cString += NfeTag('<IEST>',aIEST[01]) 
	EndIf
EndIf
...
//CUSTOMIZADO // VOLTOU EM 16/02/2016
// ORIGINAL LAU - Novel

Static Function MensOBS(cTipo)

Local cMens	:= ""
Local cMensNovel:= ""

IF cTipo == "0" .AND. !EMPTY(SF1->F1_NVIMPOB)//SA1->A1_EST == "EX"
	
	//cMens += "Prod. Importado sob regime de diferimento ICMS Decreto 4.316/95" + "-"
	If lImp
		aAux := STRTOKARR(SF1->F1_NVIMPOB,"*")
		cMens += "D.I.: " + aAux[01] + "-" 
		cMens += "Data D.I.: " + ConvType(SUBSTR(aAux[02],7,2)+"/"+SUBSTR(aAux[02],5,2)+"/"+SUBSTR(aAux[02],1,4))+ "-"  				
		cMens += "PIS: R$ " + ConvType(VAL(aAux[10]),14,2) + "-"
		cMens += "COFINS: R$ " + ConvType(VAL(aAux[11]),14,2) + "-"
		IF Val(aAux[13]) > 0
			cMens += "AFRM: R$ " + ConvType(VAL(aAux[13]),14,2) + "-"
		EndIF
		IF Val(aAux[12]) > 0
			cMens += "SISCOMEX: R$" + ConvType(VAL(aAux[12]),14,2) + "-"
		EndIf
		
		IF Val(aAux[06]) > 0
			cMens += "Armazenagem: R$" + ConvType(VAL(aAux[6]),14,2) + "-"
		EndIf
		
		IF Val(aAux[9]) > 0
			cMens += "Imposto de Importação: R$" + ConvType(VAL(aAux[9]),14,2) + "-"
		EndIf
		
		IF Val(aAux[14]) > 0
			cMens += "Outras Despesas: R$" + ConvType(VAL(aAux[14]),14,2) + "-"
		EndIf
	Else
		cMens := ALLTRIM(SF1->F1_NVIMPOB)
	EndIF
...
Else
	If !(lMvNFLeiZF)
		If (cAliasSD2)->D2_QUANT < 1		    
			...
			elseif (cAliasSD2)->D2_PRUNIT < (cAliasSD2)->D2_PRCVEN .OR.  (cAliasSD2)->D2_DESCZFR  > 0
