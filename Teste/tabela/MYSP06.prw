...
User Function MYSP06(__aCookies,__aPostParms,__nProcID,__aProcParms,__cHTTPPage)
...
cHtml	+= '<form name="form1" method="POST" >'
...
If DBSeek(xFilial("SUA")+_cOrcamento,.F.)   
   ...
   MsUnLock()  
   U_MAILDESR()
   cHtml	+= '<p class=MsoNormal><b><i>O Orcamento '+SUA->UA_NUM+' foi reprovado!!!!</i></b></p> '
else   
...
User Function MAILDESR()
	...
	Local aCorpo  	:= {}
	Local aRodape 	:= {} 
	Local cAssunto  := " Or็amento de numero "+SUA->UA_NUM+" Foi Reijeitado!" 
	...
	aAdd(aCorpo, '')
	aAdd(aCorpo, 'Foi Rejeitado pela anแlise de desconto. ')    
	aAdd(aCorpo, '')  
	aAdd(aCorpo, 'Reprovado via Web! Verifique o motivo com o responsavel') 
	aAdd(aRodape, '')
