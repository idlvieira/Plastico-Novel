...
±±ºClasse    ³TOPCREL   ºAutor  ³QSdoBrasil          º Data ³  08/11/07   º±±
±±ÌÍÍÍÍÍÍÍÍÍÍØÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍ¹±±
±±ºDesc.     ³Funcao resposavel por montar tela para usuario selecionar	  º±±
±±º          ³qual relatorio deseja visualizar							  º±±
...
User Function TOpcRel()
	...
	@ 004,004 TO 026,150 LABEL "Orçamento Plastico" PIXEL OF oDlg
...
Static Function fGeraRel(lOPlasP,lOPlasI,lOPlasE)
	...
	if !lBloq
		if lImpPlasP .or. lImpPlasI .or. lImpPlasE
			lEnvia := MSGYESNO("Deseja enviar por e-mail Orçamento ?")
			if lEnvia
...
Static Function TelaEmail(lImpPlasP,lImpPlasI,lImpPlasE)
	...
	cAssunto := "Envio de Orçamento Plástico - " + allTrim(M->UA_NOME)
	...
	DEFINE MSDIALOG oDlg2 TITLE "Orçamento - E-mail" FROM 178,181 TO 580,631 PIXEL
	
	&& Cria as Groups do Sistema
	@ 060,002 TO 155,223 LABEL "Selecione Orçamento para ser enviado como anexo no e-mail." PIXEL OF oDlg2
...
Static Function EnvEmail(oDlg2,cEndOPlasP,cEndOPlasI,cEndOPlasE,lImpPlasP,lImpPlasI,lImpPlasE,cImagem)
	...
	aAdd(aCorpo,"")
	aAdd(aCorpo,"Caso haja dúvida,favor entrar em contato.")
	aAdd(aCorpo,"")
